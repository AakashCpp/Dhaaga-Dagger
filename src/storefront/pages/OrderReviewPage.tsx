...
export function OrderReviewPage({ lines, go, placeOrder }: { lines: StoreLine[]; go: (page: StorePage) => void; placeOrder: () => string | null }) {
  const dispatch = useAppDispatch();
  const checkout = useAppSelector((state) => state.checkout);
  return <CheckoutShell step={2} title="One last look." copy="Confirm the fit, delivery details and payment preference." go={go} aside={<OrderSummary lines={lines} />}>
    ...
    <button className="primary full" onClick={() => { if (placeOrder()) go("success"); }}>Place order <ArrowRight /></button>
  </CheckoutShell>;
}

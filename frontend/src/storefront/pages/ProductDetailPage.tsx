...
export function ProductDetailPage({ product, actions, go }: { product: StoreProduct; actions: StoreActions; go: (page: StorePage) => void }) {
  ...
  const gallery = product.gallery?.length ? product.gallery : isHenley ? [product.image, "/assets/henley-material-study.png", "/assets/henley-duo-editorial.png"] : [product.image, "/assets/denim-anatomy.jpg", "/assets/denim-construction-macros.jpg"];
  ...
  return <main className="product-detail-page redesigned-detail">
    ...
    <section className="detail-layout-v2">
      ...
      <aside className="detail-buy detail-buy-v2">
        ...
        <div className="detail-promises"><p><Truck /> Free tracked delivery <small>Dispatched within 24 hours</small></p><p><RotateCcw /> 12-day return <small>No size or measurement difference is accepted. Strictly reviewed by the vendor.</small></p><p><ShieldCheck /> Base-size assurance <small>Any variance from the ordered base size is return eligible after vendor review</small></p></div>
      </aside>
    </section>

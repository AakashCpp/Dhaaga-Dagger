...
export function TrackingPage({ go }: { go: (page: StorePage) => void }) {
  ...
  return <main className="tracking-page">
    ...
    <div className="tracking-details">
      ...
      <section><h3>Order items</h3>{orderProducts.map((product) => product && <div className="mini-line" key={product.id}><img src={product.image} alt={product.name} loading="lazy" decoding="async" /><span><b>{product.name}</b><small>{product.category} / {product.subtype}</small></span><b>{money(product.price)}</b></div>)}</section>
    </div>
  </main>;
}

...
import { sendAdminOtp } from "../services/mail.service.js";
...
function adminAuthReady() {
  ...
  return env.mailMode === "smtp"
    ...
    && !env.adminJwtSecret.startsWith("development-only");
}
...
export async function requestAdminCode(request, response) {
  ...
  const expiresAt = new Date(Date.now() + env.adminOtpMinutes * 60_000);
